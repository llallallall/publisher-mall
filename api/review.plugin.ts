...
const adminSchema = gql`
    ...
    extend type Query {
        allProductReviews(skip: Int, take: Int): ProductReviewList!
    }
...
export class ReviewService {
    ...
    async findAll(ctx: RequestContext, skip = 0, take = 10) {
        const [items, totalItems] = await this.connection.getRepository(ctx, ProductReview).findAndCount({
            relations: ['product'],
            order: { createdAt: 'DESC' },
            skip,
...
export class AdminReviewResolver {
    ...
    allProductReviews(@Ctx() ctx: RequestContext, @Args() args: any) {
        return this.reviewService.findAll(ctx, args.skip, args.take);
    }
